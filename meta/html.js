...
function generateList(path) {
    ...
    for (let sub in meta.cmdObj.sub) {
        let p=path.replace('../meta','');
        p=`${p}/${sub}`;
        html += `<li class="${p}" onclick='demo(this)'>${sub}</li>\n`;
        generateContent(`${path}/${sub}`);
...
function generateContent(dirPath) {
    ...
    if (cmdObj.sub) {
        syntax += `<tr><td><code>${cmdObj.use} [subCommand]</code></td></tr>`;
    }
    if (cmdObj.options) {
    ...
    let dir=path.dirname(path.join(__dirname, dirPath.replace('meta', 'front/src')));
    if (!fs.existsSync(dir)){
        fs.mkdirSync(dir);
    }
    fs.writeFileSync(path.join(__dirname, dirPath.replace('meta', 'front/src')) + '.html', html);
}