...
exports.parser = function (args) {
  ...
  argsParse();
  let opts = exports.transOpts(cmd.cmdObj.options);
  opts = exports.transOpts(rootCmd.cmdObj.options, opts);
  exports.argv._opts = opts;
  // 解析终端所有flag
  flagsParser();
  ...
  //flag校验
  err = optionsValidate(exports.argv._opts, exports.argv._parsedValue);
  if (err) {
    exports.argv._err = err;
    return;
  }
...
exports.parseOne = function () {
  ...
  for (index in args) {
    ...
    exports.argv._opts[flagName]._input.push(args[index]);
  }
  exports.argv._args = args.slice(index);
  let value = processValue(flagName);

  exports.argv._parsedValue[exports.argv._opts[flagName].name] = value;
  if (exports.argv._opts[flagName].mapping) {
    exports.argv._mappingValue[exports.argv._opts[flagName].mapping] = value;
  }
};

function processValue(flagName) {
  let flag = exports.argv._opts[flagName];
  if (!flag.vtype) {
    flag.vtype = 'string';
  }
  let value;
...
function processString(flagName) {
  let input;
  input = exports.argv._opts[flagName]._input;
  if (input.length !== 1) {
...
function processBoolean(flagName) {
  let input = exports.argv._opts[flagName]._input;
  if (input.length === 0) {
    return true;
  }
  if (input.length === 1) {
