...
function argsValidate(argsOption, argv) {
  ...
  for (; index < argv.length; index++) {
    ...
    if (!ok) {
      let err = {
        prompt: i18n.positionArgValueErr,
        values: [arg.name, arg.choices.join('|')]
      };
      return err;
    }
  }
...
function processStringArray(flagName, input) {
  let value;
  if (input[0].startsWith('[')) {
    if (input.length !== 1) {
      let err = {
...
function processNumberArray(flagName, input) {
  if (input.length === 0) {
    let err = {
      prompt: i18n.vTypeMatchErr,
      values: [flagName]
...
function processArray(flagName, flag, input) {
  ...
  switch (flag.subType) {
  case 'string':
    result = processStringArray(flagName, input);
    break;
  case 'number':
    result = processNumberArray(flagName, input);
    break;
  case 'map':
  ...
  if (flag.maxindex) {
    if (result.value.length > flag.maxindex) {
      let err = {
