...
class Parser {
  ...
  // 主要解析逻辑
  parse() {
    const parserCtx = this.cmdParser(this.cmdFilePath);
    ...
    const cmd = require(this.cmdFilePath);
    if (this.help) {
      return;
    }
    if (!cmd.run) {
    ...
    const argvCtx = this.argvParse(this.args);
    ...
    if (this.parsedValue.profile) {
      ...
      const result = config.getProfile(this.profileName);
  ...
  cmdParser(cmdFilePath) {
    ...
    for (; index < args.length; index++) {
      ...
      const tempPath = path.join(cmdFilePath, ...cmds, arg + '.js');
    ...
    if (args.length === cmds.length) {
      ...
      newArgs = args.slice(index);
    }
    return { args: newArgs, cmds, help };
  }
