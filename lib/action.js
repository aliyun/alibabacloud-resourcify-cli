...
module.exports = class extends Command {
  ...
  async interactionOptions(ctx) {
    ...
    while (choices.length > 0) {
      const question = {
        ...
        loop: false,
        pageSize: 10,
        message: lineWrap(i18n.optionalPrompt[ctx.language])
      };
  ...
  async interactionRequireds(ctx) {
    const requireds = [];
    ...
    for (const [name, option] of Object.entries(this.def.options)) {
      if (option.required) {
        option.vtype = option.vtype || 'string';
        const ok = await this.optionInteractive(ctx, name);
        requireds.push(`--${name} ${ok}`);
      }
    }
  ...
  async interaction(ctx) {
    ...
    // requireds
    const requireds = await this.interactionRequireds(ctx);
    // options
    ...
    console.log(output);
    if (isRun) {
      await this.run(loadContext([...argv, ...requireds, ...options], this.def.options));
    }
  }
};
