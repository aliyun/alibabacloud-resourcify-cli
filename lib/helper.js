...
class Helper {
  ...
  getSyntax() {
    ...
    if (this.cmdObj.usage) {
      ...
      let message = '';
      const use = this.rootCmd + ' ' + this.cmds.join(' ');
      if (this.cmdObj.sub) {
      ...
      if (this.cmdObj.args) {
        let argsUsage = '';
        for (const arg of this.cmdObj.args) {
          let preToken = '[';
          let sufToken = ']';
          if (arg.required) {
            preToken = '<';
  ...
  getDesc(){
    if (!this.cmdObj.desc||!this.cmdObj.desc[this.language]){
      return;
    }
    this.ui.div({
      text: this.cmdObj.desc[this.language],
      padding: [1, 0, 1, 2]
    });
  }
  ...
  getOptions() {
    const result = this.parse.transOpts(this.cmdObj);
    if (result.index.length === 0) {
      return;
    }
    this.ui.div(`${i18n.optionsPrompt[this.language]}:`);
    const index = result.index;
    for (const optName of index) {
  ...
  getOneOption(name) {
    const option = this.cmdObj.options[name];
    name = '--' + name;
    let desc = '';
