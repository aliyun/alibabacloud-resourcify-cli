...
class Command {
  ...
  async handle(args) {
    ...
    if (sub && !sub.startsWith('-') && this.cmds.length > 0) {
      ...
      console.error(`'${sub}' is not a sub-command. See '${this.getCommandPath().join(' ')} help'`);
      process.exit(1);
    }
  ...
  validateOptions(parsed) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (option.required && !parsed.get(key)) {
    ...
    if (this.cmds.length > 0) {
      ...
      for (let i = 0; i < this.cmds.length; i++) {
        ...
        ui.div(
          ...
          {
            text: cmd.def.short && cmd.def.short[language] || cmd.def.desc[language]
          }
        );
      }
    }
